...
thing that non-embellished regular expressions can match), whereas
nonterminals can express a [context-free
...
You'll notice that the example has some terms starting lowercase
letter, and some that are capitalized. This difference is significant.
Capitalized names are called _tagged_, and they represent nodes in the
tree. Lower-case names are untagged, and do not appear in the output
tree.

(If you are writing rules in a script that doesn't have case, you can
use an underscore at the start of a name to indicate that the name is
untagged.)

The `top` term defines the entry point to the grammar. This is the
rule that will be used to match the entire input. It'll usually
contain something that repeats, like `statement+`.
...
Named tokens are defined in a `tokens` block. You can also, outside of
the `tokens` block, use string literals like `"+"` as tokens. These
automatically define a new token, which is tagged, using the
string—including the quotes—as its tag name. String literals use the
same escaping rules as in JavaScript.

String literals inside of token rules mean something very different.
They are just a part of the token, not the entire thing. Token rules
are compiled in a different way than nonterminal rules. The parser
generator builds a [deterministic finite
automaton](https://en.wikipedia.org/wiki/Deterministic_finite_automaton)
from all the token rules, which can then be used to efficiently match
them against a text stream.
...
Token rules can not refer to nonterminal rules. But they can refer to
...
This is defined like a regular nonterminal.
...
us that, after reading what looks like a expression and seeing another
operator, it doesn't know whether to first reduce the initial tokens
to a `BinaryExpression` or to first shift the second operator and
leave the first for later. In short, it doesn't know whether to parse
`x + y + z` as `(x + y) + z` or `x + (y + z)`.
...
block that enumerates the precedences you are going to use in the
...
given precedence, to make it a cut operator by using the word `cut`. A
cut operator will override other interpretations even though no
conflict was detected yet. An example of where this is appropriate is
the way JavaScript's `function` keyword has a different meaning in
...
This will parse `function...` as a `FunctionDeclaration` node, though
it would also match `FunctionExpression` (which could be used
elsewhere in the grammar, if it was a real grammar).
...
order to know whether to create a `GoodValue` or `BadValue` node, the
parser has to decide whether it is parsing a `GoodStatement` or a
...
approaches for the next token, and then drop off one path when it sees
that it doesn't allow the next token.
...
When you pass arguments to a rule defined in the `tokens` block, those
arguments will be interpreted as if they are part of a token rule,
even when you call it from a nonterminal rule.
...
To explicitly specify that, in such a case, one of the tokens take
...
`precedence` declarations in your `tokens` block, and each declaration
can contain any number of tokens, breaking ties between those tokens.
...
```
NewExpression { specialize<Identifier, "new"> Identifier }
...
`specialize` takes an optional third argument to assign a tag to the
specialized token. It is often useful to define a helper rule like
this to easily define keywords.
...
Multiple terms are allowed to share a single tag. For example, you
might want to add the `Operator` tag to various types of operators.
You can't define multiple rules named `Operator`, but you _can_
annotate rules with another name to have a given tag associated with
their term.
...
Any rules (both token and nonterminal rules) can have an `=` and a tag
name after their rule name.

It is also possible to define tagged rules inline in other rules using
`tag.Tagname` syntax (where `Tagname` can be any word).

```
expression { Number | tag.PlusExpression<expression "+" expression> }
```

The convention is to, when possible, follow the naming style used in
the [ESTree](https://github.com/estree/estree/blob/master/es2015.md)
JavaScript syntax tree spec. Since most languages are full of
constructs that do not exist in JavaScript, that can only be a source
of inspiration, not a rigorous rule. If you can find a Lezer grammar
for another language similar to the one your are defining, you are
encouraged to borrow tag names from that.

### External Tokens

The regular way to define tokens is usually enough, and convenient to
add. But regular languages are notoriously limited in what they can
match, so some things, like the multiple-character look-ahead needed to
recognize the end of a comment, are terribly awkward to express inside
the grammar. And something like JavaScript's automatic semicolon
insertion, which requires checking the content of preceding whitespace
and looking at the character ahead, is just impossible to express like
that.
...
a tag name to assign to the node that wraps the nested region. The
second may be an explicit end token. By default, the token occurring
directly after the `nest` expression is taken as the token that ends
the region, but sometimes that isn't what you want. Finally, a third
...
instances, which represent an actual single parse state. When the
parse is unambiguous, there'll be one stack. When it is trying out
...
function](##lezer.NestedGrammar) gets access to it and can query it
to, for example, ask if it accepts a given
[token](##lezer.Stack.canShift) or where the rule that is currently
being matched [starts](##lezer.Stack.ruleStart).

Parsing consumes an
[`InputStream`](http://localhost:5000/docs/ref/#lezer.InputStream),
which abstracts access to a string-like data structure. You may simply
pass a string to [`parse`](##lezer.Parser.parse), in which case the
library will do the wrapping itself, but if you have your data in some
kind of data structure more involved than a string, you'll need to
write your own class the implements
[`InputStream`](http://localhost:5000/docs/ref/#lezer.InputStream), so
that Lezer can read directly from your data structure, without copying
its content into a flat string.
